...
## creates a copy of file_letter but in array form and sorted
function print_file_letters() {
	for key in "${!file_letters[@]}" ; do
		printf "%s %d\n" "$key" "${file_letters[$key]}"
	done | sort -rn -k2
}

function print_sorted_values2() {
...
function to_array() {
        ...
        mapfile -d '' sorted < <(printf '%d\0' "${file_letters[@]}" | sort -z)

        sorted=()
        while IFS= read -r row; do
                key=$(echo $row | cut -d " " -f 1)
                sorted+=( "$key" )
        done < <(print_sorted_values2)
	...
	done < <(print_file_letters)
}

function print_sorted_values1() {
        for key in "${sorted[@]}"; do
                printf '%s %.3f\n' "$key" "${db_letters[$key]}"
        done | sort -rn -k2
}
